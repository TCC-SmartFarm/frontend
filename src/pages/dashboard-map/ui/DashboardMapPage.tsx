...
import {
  ...
  statusFor,
  type ParamKey,
} from "@/shared/constants/sensor-mock";
...
export const DashboardMapPage = () => {
  ...
  useEffect(() => {
    ...
    SENSORS.forEach((sensor) => {
      ...
      marker.bindTooltip(buildTooltipHTML(sensor), {
        direction: "top",
        opacity: 1,
      });
      marker.bindPopup(buildPopupHTML(sensor), {
        minWidth: 260,
        maxWidth: 280,
      });
  ...
  const selHasIssue = METRICS.some(
    (m) => statusFor(m.key, selR[m.key]) !== "ok",
  );
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "bg-leaf-50/80 hover:bg-leaf-50"
                      : "hover:bg-sand-50/80",
                  )}
                >
